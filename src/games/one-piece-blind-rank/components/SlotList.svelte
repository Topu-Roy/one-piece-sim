...
<ol class="flex w-full flex-col gap-2">
  {#each placements as placement (placement.slot)}
    ...
    <li>
      ...
      {:else}
        <button
          class="flex w-full items-center gap-3 rounded-2xl border-[3px] border-dashed border-cocoa/50 bg-white/60 px-3 py-2 text-left text-cocoa shadow-[4px_4px_0_rgba(58,42,24,0.25)] active:translate-x-0.5 active:translate-y-0.5 active:shadow-none"
        ...
        >
          ...
          <span class="flex-1 text-sm font-bold text-cocoa/40">Empty — tap to place here</span>
        </button>
      {/if}
    </li>
  {/each}
